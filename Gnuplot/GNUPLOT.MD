...
<img src="./files/tutorial_gnuplot/terminal_gnuplot_success.png" width="800" title="terminal_gnuplot_fail">

Em caso contrário, execute o comando sugerido no terminal:

<img src="./files/tutorial_gnuplot/terminal_gnuplot_fail.png" width="800" title="terminal_gnuplot_fail">
...
> [!IMPORTANT]
> Para aplicar as alterações feitas com `set`, é necessário executar o comando `gnuplot> replot`, garantindo a atualização do gráfico. O mesmo princípio vale quando se deseja gerar dois ou mais gráficos simultaneamente: após utilizar plot ou splot, os gráficos seguintes devem ser gerados com replot.

## Gnuplot em C

Por ser um software independente (um programa instalado no seu PC) o C não importa uma biblioteca específica para usá-lo, ocorre a  utilização da própria stdio.h para fazer a abertura de um "cano/tubulação" (pipe) que através do seu código C envia comandos de texto que o Gnuplot executa e renderiza em tempo real.
...
| **Abertura**       | fopen("arquivo.txt", "modo");  | popen("gnuplot", "modo"); |
...
>[!NOTE] 
> No argumento "gnuplot", é comum adicionarmos a flag -persist (ficando "gnuplot -persist"). Isso informa ao sistema que a janela do gráfico deve continuar aberta mesmo após o seu programa em C ser finalizado.
...
>[!NOTE] 
> Todo comando enviado ao gnuplot deve terminar com '\n'.

### Enviando as coordenadas
O Gnuplot interpreta as informações recebidas (seja via arquivo .txt ou diretamente da memória RAM) como uma tabela organizada em colunas. Por padrão, o Gnuplot associa a primeira coluna de dados ao eixo X e a segunda coluna ao eixo Y. Caso se envie uma linha com os valores 4.5 e 1.2, o Gnuplot entenderá que 4.5 é a Coluna 1 (X) e 1.2 é a Coluna 2 (Y).

### Mudando a Ordem com o using
Caso você queira alterar qual dado vai para qual eixo sem precisar mudar a ordem no seu código C, utilize o parâmetro using. Ele permite mapear as colunas da forma que você desejar seguindo o formato `using <eixo X>:<eixo Y>`. Exemplo:
...
Para isso, utilizamos o parâmetro with lines (ou a abreviação w l) logo após o comando de plotagem:`fprintf(gnuplot_pipeMP, "plot '-' with lines \n");`

>[!NOTE]
> É possível configurar o set title, set grid, etc, em linhas diferentes (antes do plot). Mas o using, o with lines e o title fazem parte do comando plot. Eles são paramêtros do plot e devem ir juntos com ele.
...
int main() {
    ...
    // 3. Configurar e Plotar
    fprintf(pipe, "set title 'Exemplo Simples'\n");
    fprintf(pipe, "set grid\n");
    fprintf(pipe, "plot 'dados.txt' with lines\n");
...
Relembrando as Tarefas 1 e 2, foi utilizado a estrutura `astro_base_ackerman_motion_command_message` para lidar com as mensagens do sistema. Agora, você usará essa mesma struct para monitorar o movimento do veículo:

1 - Coleta e Registro: Capture a velocidade (v) e o timestamp de cada mensagem recebida e armazene esses dados em um arquivo .txt enquanto o simulador Navigate (Tarefa 2) estiver em execução.

2 - Visualização: Após concluir a coleta, utilize o Gnuplot para ler o arquivo gerado e criar um gráfico de velocidade x tempo, seguindo o modelo da imagem abaixo:
